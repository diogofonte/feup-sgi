    ...
    */ 
    ...
    createCameras(cameras){
        ...
        for (var key in cameras){
            ...
            switch(cameras[key].type){
                case 'orthogonal':
                    let orthogonal = new THREE.OrthographicCamera(cameras[key].left, cameras[key].right, cameras[key].top, cameras[key].bottom, cameras[key].near, cameras[key].far);
                    orthogonal.position.set(cameras[key].location[0], cameras[key].location[1], cameras[key].location[2])
                    target = new THREE.Vector3(cameras[key].target[0], cameras[key].target[1], cameras[key].target[2])
                    orthogonal.lookAt(target)
                    this.camerasTargets[cameras[key].id] = target
                    this.cameras[cameras[key].id] = orthogonal
                    break;
                case 'perspective':
                    let perspective = new THREE.PerspectiveCamera(cameras[key].angle, window.innerWidth / window.innerHeight, cameras[key].near, cameras[key].far);
                    perspective.position.set(cameras[key].location[0], cameras[key].location[1], cameras[key].location[2])
                    target = new THREE.Vector3(cameras[key].target[0], cameras[key].target[1], cameras[key].target[2])
                    perspective.lookAt(target)
                    this.camerasTargets[cameras[key].id] = target
                    this.cameras[cameras[key].id] = perspective
                    break;
                default:
                    break;
            }
    ...
    updateLight(value, light){

        if(value) this.app.scene.add(this.lights[light])
        else this.app.scene.remove(this.lights[light])

    }

    updateWireframes(value){

        if(value) this.wireframesEnabled = true
        else this.wireframesEnabled = false

        if(this.wireframesEnabled){
            for(let material in this.materialDictionary){
                if(this.wireframes[material]) this.materialDictionary[material].wireframe = true;
            }
        } else {
            for(let material in this.materialDictionary){
                if(this.wireframes[material]) this.materialDictionary[material].wireframe = false;
            }
        }
